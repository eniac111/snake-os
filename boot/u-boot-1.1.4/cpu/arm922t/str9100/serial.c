...
void star_serial_config_baud_rate(u32 baud_rate, u32 input_clock)
{
	...
	if (baud_rate == 1500000) {
		HAL_UART_WRITE_PSR(1);
	} else {
		HAL_UART_WRITE_PSR(2);
	}
	updated_input_clock = (input_clock / 325) * 100;
	...
	if (baud_rate_divisor) {    
		HAL_UART_WRITE_DLM(DLM);
		HAL_UART_WRITE_DLL(DLL);
	}
}

void star_serial_config_line_control(u32 word_len, u32 parity, u32 stop_bit)
{
	u32 uart_LCR = 0;
...
void star_serial_config_fifo(u32 fifo_flag, u32 rx_fifo_trigger_level)
{
	u32 uart_FCR = 0;
...
void star_serial_hw_init(void)
{
	...
	star_powermgt_soft_reset(PWRMGT_UART0_SOFTWARE_RESET_BIT_INDEX);
	HAL_PWRMGT_ENABLE_UART1_CLOCK();
	star_powermgt_soft_reset(PWRMGT_UART1_SOFTWARE_RESET_BIT_INDEX);
#endif
}
...
void serial_setbrg(void)
{
	...
	star_serial_hw_init();
	star_serial_config_baud_rate(gd->baudrate, UART_INPUT_CLOCK);
	star_serial_config_line_control(WORD_EIGHT_BITS, NONE_PARITY, ONE_STOP_BIT);
	star_serial_config_fifo(TX_RX_FIFO_ENABLE, 8);
}
...
int serial_getc(void)
{
	...
	while (!HAL_UART_CHECK_RX_DATA_READY())
		; /* wait */

	HAL_UART_READ_DATA(rx_data);
...
{
	while (!HAL_UART_CHECK_TX_FIFO_EMPTY())
		; /* wait */

	HAL_UART_WRITE_DATA(c);
	if (c == '\n') {
		serial_putc('\r');
	}
}
...
{
	return (HAL_UART_CHECK_RX_DATA_READY());
}

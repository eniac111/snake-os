...
#ifdef CONFIG_STR8133	/* eric@01082008 */
struct pio_timing_t
{
	u32 T2;
	u32 T1;
...
/* ------------------------------------------------------------------------- */
#ifdef CONFIG_STR8133
static inline int str8131_ide_is_device_ready(void)
{
	return ((_IDE_STATUS_REG & IDE_BSY) ? 0 : 1);
}

static int str8131_ide_soft_reset(void)
{
	int i;

	for (i = 1000; i > 0; i--) {
		if (str8131_ide_is_device_ready())
			break;
		udelay(1000);
	}
	if (i == 0) return 0;
...
void ide_init (void)
{
	...
	if (!str8131_ide_soft_reset()) {
		printf("STR8131 IDE: Software Reset Failed!!\n");
		return ;
	}
#endif
#ifdef CONFIG_IDE_8xx_DIRECT
...
#if defined(__PPC__) || defined(CONFIG_PXA_PCMCIA) || defined(CONFIG_STR8133)
...
#ifndef	CONFIG_STR8133	/* eric@01082008 */
	ide_set_reset (1); 
#endif

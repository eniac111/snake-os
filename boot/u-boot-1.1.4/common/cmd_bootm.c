...
int do_bootusbd (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int rcode = 0;
	if(!(PWRMGT_USB_DEVICE_POWERMGT_REG&0x1)) {
		printf("%s: usbd not attached to host...\n",__FUNCTION__);
		return rcode;
	}
	printf("%s: usbd attached to host...\n",__FUNCTION__);
#ifndef CFG_HUSH_PARSER
	if (run_command (getenv ("bootusbdcmd"), flag) < 0) rcode = 1;
#else
	if (parse_string_outer(getenv("bootusbdcmd"),
		FLAG_PARSE_SEMICOLON | FLAG_EXIT_FROM_LOOP) != 0 ) rcode = 1;
#endif
	return rcode;
}
...
U_BOOT_CMD(
 	bootusbd,	1,	1,	do_bootusbd,
 	"bootusbd    - boot default, i.e., run 'bootusbdcmd'\n",
	NULL
);

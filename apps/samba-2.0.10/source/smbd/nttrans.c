							...
							uint32 desired_access, uint32 share_access, uint32 file_attributes)
{
  ...
  *pstat_open_only = False;
  ...
  if (smb_open_mode == -1) {
	...
	/* 
	* SNAKE OS - NT seems to sometimes send desired_access as zero. play it safe
	* and map to a stat open.
	*/
	if(desired_access == 0 || desired_access == 2147483648 ) {
		smb_open_mode = DOS_OPEN_RDONLY;
	}
	else if(desired_access == 1073741824) {
		smb_open_mode = DOS_OPEN_WRONLY;
	}
	else if(desired_access == 3221225472) {
    smb_open_mode = DOS_OPEN_RDWR;
	}
    else if(desired_access & (DELETE_ACCESS|WRITE_DAC_ACCESS|WRITE_OWNER_ACCESS|SYNCHRONIZE_ACCESS|
